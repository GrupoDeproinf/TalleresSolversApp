...
const nivelBlindajeOptions = ['Básico', 'Intermedio', 'Alto', 'Máximo'];
...
const VehicleAddStepper = () => {
    ...
    setForm(
      initialValues && typeof initialValues === 'object'
        ? {
            ...
            litros_aceite: String(initialValues?.litros_aceite ?? ''),
            tipo_aceite_diferencial: String(initialValues?.tipo_aceite_diferencial ?? ''),
            viscosidad_aceite_diferencial: String(initialValues?.viscosidad_aceite_diferencial ?? ''),
            ...
            marca_bujia: String(initialValues?.marca_bujia ?? ''),
            codigo_bujia: String(initialValues?.codigo_bujia ?? ''),
    ...
    if (!uri) {
      return (
        <TouchableOpacity
          ...
          onPress={() => pickDocumentFromGallery(uriKey, b64Key)}
          activeOpacity={0.85}>
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      litros_aceite: (form.litros_aceite || '').trim() || null,
      tipo_aceite_diferencial: (form.tipo_aceite_diferencial || '').trim() || null,
      viscosidad_aceite_diferencial: (form.viscosidad_aceite_diferencial || '').trim() || null,
      ...
      marca_bujia: (form.marca_bujia || '').trim() || null,
      codigo_bujia: (form.codigo_bujia || '').trim() || null,
            ...
            <View style={styles.kmHeroOuter}>
              <View style={styles.kmHeroTouchable}>
                ...
                {/* <View style={styles.kmHeroChevron}>
                  <IconsFA5 name="pen" size={14} color="#FFD60A" />
                </View> */}
              </View>
            </View>
          ) : null}
