...
const TallerDetail = () => {
  ...
  const heroImageSource =
    taller?.image_perfil && String(taller.image_perfil).trim() !== ""
      ? { uri: taller.image_perfil }
      : require("../../assets/noimageNew.png");

  const instalacionesImages = useMemo(() => {
    const images = [];
    if (taller?.fotoFrenteTaller) images.push({ uri: taller.fotoFrenteTaller });
    if (taller?.fotoInternaTaller) images.push({ uri: taller.fotoInternaTaller });
    if (images.length > 0) return images;

    if (Array.isArray(taller?.instalaciones_images) && taller.instalaciones_images.length > 0) {
      return [{ uri: taller.instalaciones_images[0] }];
    }
    if (Array.isArray(taller?.galeria) && taller.galeria.length > 0) {
      return [{ uri: taller.galeria[0] }];
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <MapboxNavigation
        ...
        destinationName={taller?.nombre || 'Taller'}
