...
const ServiciosContainer = ({ navigation }) => {
  ...
  const registroRedirectedRef = useRef(false);
  ...
  const filteredServicios = useMemo(() => {
    if (filterCategoria === 'Todos') return dataServicios;
    return dataServicios.filter(s => String(s.categoria || '').trim() === filterCategoria);
  }, [dataServicios, filterCategoria]);

  const totalServicios = dataServicios.length;
  const publicados = dataServicios.filter(s => s.estatus).length;
  const noPublicados = dataServicios.filter(s => !s.estatus).length;


  const navigationScreen = useNavigation();

  // uid_taller / nombre_taller recibidos desde FormTaller (agente viendo servicios de otro taller)
  const uidTallerParam    = route.params?.uid_taller;
  const nombreTallerParam = route.params?.nombre_taller;

  // Cargar datos en el montaje inicial
  useEffect(() => {
    if (uidTallerParam) {
      // Modo agente: mostrar directamente los servicios del taller indicado
      setshowServices(true);
      getServices(uidTallerParam);
    } else {
      getData();
    }
  }, []);
  ...
  // Flujo de registro: cierra modales y va directo a crear servicio
  useEffect(() => {
    if (route.params?.fromRegistro === true && !registroRedirectedRef.current) {
      registroRedirectedRef.current = true;
      setShowModalAprobacion(false);
      setShowModalCitaAgendada(false);
      ...
      try {
        ...
        if (result.message === 'Usuario encontrado') {
          ...
          if (result?.userData?.status == "En espera por aprobación" && result?.userData?.scheduled_visit == undefined) {
            if (!registroRedirectedRef.current) {
            ...
            if (!registroRedirectedRef.current) setShowModalCitaAgendada(true);
  ...
  // Modal post-registro: primer servicio guardado
  const renderModalRegistro = () => (
    <Modal
      ...
      <View style={modalStyles.centeredView}>
        <View style={[modalStyles.modalView, { paddingHorizontal: 24, paddingVertical: 28 }]}>
          <Text style={{ fontSize: 40, textAlign: 'center', marginBottom: 8 }}>🏁</Text>

          <Text style={[modalStyles.titleText, { textAlign: 'center', marginBottom: 10 }]}>
            ¡Servicio guardado!
          </Text>

          <Text style={[modalStyles.descriptionText, { textAlign: 'center', marginBottom: 8 }]}>
            Tu negocio ya está en lista de espera para aprobación.
          </Text>

          <View style={[modalStyles.warningBox, { marginBottom: 14 }]}>
            <Text style={{ fontSize: 20, marginRight: 8 }}>📞</Text>
            <Text style={[modalStyles.warningText, { flex: 1 }]}>
              Pronto te contactaremos para activar tu cuenta.
            </Text>
          </View>

          <View style={[modalStyles.warningBox, { backgroundColor: '#EEF2FF', borderColor: '#C7D2FE', marginBottom: 20 }]}>
            <Text style={{ fontSize: 20, marginRight: 8 }}>➕</Text>
            <Text style={[modalStyles.warningText, { flex: 1, color: '#3730A3' }]}>
              ¡Adelanta trabajo! Usa el botón <Text style={{ fontWeight: 'bold' }}>+</Text> para seguir agregando servicios a tu catálogo.
            </Text>
          </View>

          <TouchableOpacity
            style={[modalStyles.button, { backgroundColor: '#1D1E56' }]}
            activeOpacity={0.9}
            onPress={() => {
              setShowRegistroModal(false);
              navigationScreen.navigate('FormService', { uid: '' });
            }}>
            <Text style={[modalStyles.buttonText, { color: '#FFD60A' }]}>¡Entendido!</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
  ...
  if (showServices) {
    return (
      <View
        ...
        {(dataServicios.length > 0 || uidTallerParam) && <View style={[modalStyles.servicesHeaderWrapper, { paddingTop: insets.top + windowHeight(3.8) }]}>
          ...
          {uidTallerParam ? (
            <TouchableOpacity
              ...
              style={modalStyles.servicesHeaderBackBtn}
              activeOpacity={0.85}>
          ...
          <View style={modalStyles.servicesHeaderRow}>
            <View style={modalStyles.servicesHeaderCenter}>
              ...
              <Text style={modalStyles.servicesHeaderSubtitle}>
                Gestiona y publica tus servicios para conectar con más clientes.
              </Text>
            </View>
          </View>
        </View>}
        ...
        {dataServicios.length > 0 ? (
          <>
            ...
            <View style={modalStyles.statsCard}>
              <View style={modalStyles.statsMetricRow}>
                <View style={modalStyles.statsMetricCard}>
                  <Text style={modalStyles.statsMetricValue}>{totalServicios}</Text>
                  <Text style={modalStyles.statsMetricLabel}>Total</Text>
                </View>
                <View style={[modalStyles.statsMetricCard, modalStyles.statsMetricCardYellow]}>
                  <Text style={[modalStyles.statsMetricValue, modalStyles.statsMetricValueGreen]}>
                    {publicados}
                  </Text>
                  <Text style={[modalStyles.statsMetricLabel, modalStyles.statsMetricLabelDark]}>
                    Publicados
                  </Text>
                </View>
                <View style={[modalStyles.statsMetricCard, modalStyles.statsMetricCardRed]}>
                  <Text style={[modalStyles.statsMetricValue, modalStyles.statsMetricValueRed]}>
                    {noPublicados}
                  </Text>
                  <Text style={modalStyles.statsMetricLabel}>No publicados</Text>
                </View>
              </View>
            </View>
            ...
            <ScrollView
              ...
              style={modalStyles.filterRow}
              contentContainerStyle={modalStyles.filterRowContent}>
              {categorias.map(cat => (
                <TouchableOpacity
                  key={cat}
                  style={[modalStyles.filterChip, filterCategoria === cat && modalStyles.filterChipActive]}
                  onPress={() => setFilterCategoria(cat)}
                  activeOpacity={0.8}>
                  <Text style={[modalStyles.filterChipText, filterCategoria === cat && modalStyles.filterChipTextActive]}>
        ...
        {dataServicios.length === 0 ? (
          uidTallerParam ? (
            /* ── Empty state: taller sin servicios (vista desde Acciones) ── */
            <View style={modalStyles.emptyTallerContainer}>
              ...
              <Text style={modalStyles.emptyTallerTitle}>Sin servicios</Text>
              <Text style={modalStyles.emptyTallerSubtitle}>
                Este taller aún no tiene{'\n'}servicios registrados.
              </Text>
              <TouchableOpacity
                style={modalStyles.emptyTallerBackBtn}
                onPress={() => navigationScreen.goBack()}
                activeOpacity={0.85}>
                <Icons name="angle-left" size={15} color="#FFFFFF" />
                <Text style={modalStyles.emptyTallerBackText}>Volver al taller</Text>
              </TouchableOpacity>
            </View>
          ) : (
          ...
          <ScrollView
            ...
            <ServicesContainer
              ...
              show={false}
              showPlus={true}
              uidTaller={uidTallerParam}
              nombreTaller={nombreTallerParam}
            />
          </ScrollView>
        )}
  ...
  // Ningún flag activo aún: pantalla en blanco mientras terminan las validaciones
  return null;
...
const modalStyles = StyleSheet.create({
  ...
  // ── Stats card (standalone, below header) ───────────────────────────────────
  statsCard: {
    marginHorizontal: 16,
    marginTop: 14,
    backgroundColor: '#1F2344',
  ...
  statsMetricLabelDark: {
    color: 'rgba(31,35,68,0.65)',
  },
