...
import MapboxTalleresMap from './components/MapboxTalleresMap';
...
const PerimeterMapScreen = () => {
  ...
  // Agrega lat/lng al nivel raíz para el mapa Mapbox.
  // Intenta varios nombres de campo para ser robusto ante distintos formatos del backend.
  const getTalleresForMap = () =>
    getFilteredTalleres()
      .map(t => {
  ...
  useEffect(() => {
    const fetchNearbyTalleres = async () => {
      ...
      try {
        ...
        const lista = response.data.talleres || response.data || [];
        // LOG TEMPORAL: muestra el campo ubicacion del primer taller para confirmar nombres de campo
        if (lista.length > 0) {
          console.log('🗺️ ubicacion primer taller:', JSON.stringify(lista[0].ubicacion));
        }
        setTalleres(lista);
      } catch (error) {
  ...
  return (
    <>
      ...
      {/* Mapa Mapbox con markers — sin Google Maps API key */}
      <MapboxTalleresMap
        visible={showMap}
        talleres={getTalleresForMap()}
        userLocation={location}
        onClose={() => setShowMap(false)}
        onTallerPress={taller => {
          setShowMap(false);
          handleTallerPress(taller);
        }}
      />
